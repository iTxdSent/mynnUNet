...
class HFFE(nn.Module):
    ...
    def forward(self, x_low: torch.Tensor, x_high: torch.Tensor) -> torch.Tensor:
        ...
        # Eq.(9)(10)
        f_low_p = self.cbr3_low(self.sam_low(x_low)) #Flow'
        f_high_p = self.cbr3_high(self.sam_high(x_high_up)) #Fhigh'

        # Eq.(11)(12)
        swm_low = self.sigmoid(self.swm_low_conv(f_low_p))       # SAMlow,[B,1,H,W]
        swm_high = self.sigmoid(self.swm_high_conv(f_high_p))    # SAMhigh,[B,1,H,W]

        # Eq.(13)(14): cross recalibration
        f_low_pp = x_high_up * swm_low
        f_high_pp = x_low * swm_high

        # Eq15-16 原版，公式写错了，应该是pp：
        fuse_mid = self.fuse_pre(torch.cat([f_low_pp, f_high_pp], dim=1))
        swm_fuse = self.coord_att_map(fuse_mid)  # SAMfuse，[B,out,H,W] in [0,1]加了一个温度控制一下
