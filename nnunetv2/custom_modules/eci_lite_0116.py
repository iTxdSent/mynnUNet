...
class ECILiteConfig:
    ...
    Contract:
      - Input  x: (B,C,H,W)
      - Output y: (B,C,H,W)   (same shape, same channels)

    Mechanism:
      - Build an edge feature branch E(x) cheaply
      - Compute:
          U = 1 - sigmoid(g_main(x))          # "seg uncertainty gate"
          A = sigmoid(g_edge(E(x)))           # "edge saliency gate"
          inj = (U*A) * proj(E(x))
          y = x + scale * inj
      - Optional auxiliary output: edge_logits = head(E(x)) for supervision.

    Notes:
      - No extra heavy conv stack. No SE. No RDF.
      - scale is runtime-settable to allow warm-up/ramp (trainer control).
    """
    ...
    detach_edge_for_injection: bool = False
...
class ECILite(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        edge_feat_inj = edge_feat.detach() if self.cfg.detach_edge_for_injection else edge_feat
